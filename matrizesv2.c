    ...
    int main (){
      ...
      zerarMat (mundo);
      scanMat (mundo);
      while (compMats (mundo, estadoAnterior) == 0){
          printMat (mundo);
          novaGeracao (mundo, estadoAnterior);
        }
    }
          ...
          for (j = 0; j < TAM; j++){
              if (novoValor != -1){
                  while (novoValor != -1 && novoValor != 0 && novoValor != 1){//Garantir que os únicos valores aceitos sejam 0, 1 ou -1
                      ...
                      printf ("\n Para encerrar a digitaC'C#o, digite -1.\n");
                      scanf ("%d", &novoValor);
                    }
    ...
    int somarVizinhos (int matriz[][TAM], int i, int j){
      ...
      if (i == 0){ //Elemento está na primeira linha
          if (j == 0){ //i=0 j=0
            ...
            } else { //Elemento está na primeira linha e numa coluna que nC#o C) importante
          ...
          if (j == 0){ //Elemento está na última linha e na primeira coluna
              somaVizinhos =
                matriz[TAM - 2][TAM - 1] + matriz[TAM - 2][0] + matriz[TAM - 2][1] + matriz[TAM - 1][TAM - 1] + matriz[TAM - 1][1] + matriz[0][TAM - 1] + matriz[0][0] + matriz[0][1];
          ...
          if (j == 0){//Elemento está numa linha irrelevante e na primeira coluna
            ...
            } else { //Elemento não está em uma linha ou coluna importante
                   ...
                   for (j = 0; j < TAM; j++){
                      matrizAnterior[i][j] = matriz[i][j];
                        switch (somarVizinhos (matriz, i, j)){ 
                          case 0 ... 2: //Qualquer célula viva com menos de dois vizinhos vivos morre de solidão.
                          matriz[i][j] = somarVizinhos (matriz, i, j); 
                          break;
